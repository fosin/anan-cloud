...
public interface UserFeignService {
    ...
    SingleResult<UserDTO> findOneById(@PathVariable(TreeVO.ID_NAME) Long id, @RequestParam(PathPrefixConstant.API_VERSION_NAME) String version);

    @PostMapping({PathConstant.PATH_IDS})
    MultResult<UserDTO> listByIds(@RequestBody List<Long> ids, @RequestParam(PathPrefixConstant.API_VERSION_NAME) String version);

    @GetMapping("/list/organizId/{organizId}/{status}")
    MultResult<UserDTO> listByOrganizId(@PathVariable("organizId") Long organizId,
                                        @PathVariable("status") Integer status,
                                        @RequestParam(PathPrefixConstant.API_VERSION_NAME) String version);

    @GetMapping("/list/topId/{topId}/{status}")
    MultResult<UserDTO> listAllChildByTopId(@PathVariable("topId") Long topId,
                                            @PathVariable("status") Integer status,
                                            @RequestParam(PathPrefixConstant.API_VERSION_NAME) String version);
}
