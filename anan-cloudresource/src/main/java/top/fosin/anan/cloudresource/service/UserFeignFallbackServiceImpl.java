...
public class UserFeignFallbackServiceImpl implements UserFeignService {
    ...
    @Override
    public ResponseEntity<AnanUserRespDto> findOneById(@PathVariable(TreeDto.ID_NAME) Long id) {
        log.error("feign 远程获取用户信息失败:{}", id);
        return null;
    }

    @Override
    public ResponseEntity<List<AnanUserRespDto>> listByIds(List<Long> ids) {
        log.error("feign 远程获取用户信息失败:{}", ids);
        return null;
    }

    @Override
    public ResponseEntity<AnanUserRespDto> findOneByUsercode(@PathVariable("usercode") String usercode) {
        log.error("feign 远程获取用户信息失败:{}", usercode);
        return null;
    }

    @Override
    public ResponseEntity<List<AnanUserRespDto>> listByOrganizId(@PathVariable("organizId") Long organizId, @PathVariable("status") Integer status) {
