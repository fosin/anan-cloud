...
public interface OrgFeignService {
    ...
    SingleResult<OrgRespDto> findOneById(@PathVariable(IdProp.ID_NAME) Long id, @RequestParam(PathPrefixConstant.API_VERSION_NAME) String version);

    @PostMapping({PathConstant.PATH_IDS})
    @ApiOperation("根据用户序号集合远程查询多条数据")
    MultResult<OrgRespDto> listByIds(@RequestBody List<Long> ids, @RequestParam(PathPrefixConstant.API_VERSION_NAME) String version);
    ...
    MultResult<OrgRespDto> listChild(@PathVariable(PidProp.PID_NAME) Long pid, @RequestParam(PathPrefixConstant.API_VERSION_NAME) String version);
    ...
    @ApiImplicitParam(name = PidProp.PID_NAME, value = "根据父序号pid远程其所有孩子数据集合",
            paramType = "path", required = true, dataTypeClass = Long.class)
    MultResult<OrgRespDto> listAllChild(@PathVariable(PidProp.PID_NAME) Long pid, @RequestParam(PathPrefixConstant.API_VERSION_NAME) String version);
    ...
    @ApiImplicitParam(name = PidProp.PID_NAME, value = "根据父序号pid远程其所有孩子数据，并构建树型对象",
            paramType = "path", required = true, dataTypeClass = Long.class)
    MultResult<OrgRespDto> treeAllChild(@PathVariable(PidProp.PID_NAME) Long pid, @RequestParam(PathPrefixConstant.API_VERSION_NAME) String version);
