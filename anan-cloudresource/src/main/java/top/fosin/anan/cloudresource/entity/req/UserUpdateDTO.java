...
@Schema(description = "系统用户表(anan_user)更新DTO")
public class UserUpdateDTO extends Id<Long> {
    ...
    @NotBlank(message = "用户工号" + "{jakarta.validation.constraints.NotBlank.message}", groups = Update.class)
    @Schema(description = "用户工号", example = "String")
    @Pattern(regexp = "[A-Za-z][A-Za-z0-9]{1,30}", message = "用户工号只能大小写字母开头，数字、下杠(_)组合而成,长度不超过30位", groups = Update.class)
    private String usercode;

    @NotBlank(message = "用户姓名" + "{jakarta.validation.constraints.NotBlank.message}", groups = Update.class)
    @Schema(description = "用户姓名", example = "String")
    @Pattern(regexp = RegexUtil.SPECIAL, message = "用户姓名不能包含特殊字符", groups = Update.class)
    ...
    @Past(message = "生日" + "{jakarta.validation.constraints.Past.message}", groups = Update.class)
    @NotNull(message = "生日" + "{jakarta.validation.constraints.NotNull.message}", groups = Update.class)
    private Date birthday;

    @NotNull(message = "使用状态：具体取值于字典表anan_dictionary.code=15" + "{jakarta.validation.constraints.NotNull.message}", groups = Update.class)
    ...
    @Pattern(regexp = RegexUtil.PHONE_ZH_CN, message = "手机号码{jakarta.validation.constraints.Pattern.message}", groups = Create.class)
    private String phone;

    @NotNull(message = "使用状态：0=启用，1=禁用，具体取值于字典表anan_dictionary.code=11" + "{jakarta.validation.constraints.NotNull.message}", groups = Update.class)
    ...
    @NotNull(message = "过期时间" + "{jakarta.validation.constraints.NotNull.message}", groups = Create.class)
