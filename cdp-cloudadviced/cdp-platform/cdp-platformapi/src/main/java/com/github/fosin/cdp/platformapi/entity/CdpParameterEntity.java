...
@DynamicUpdate
@SQLDelete(sql = "update cdp_parameter set deleted = 1 where id = ?")
@Where(clause = "deleted = 0")
@Table(name = "cdp_parameter")
@ApiModel(value = "用于存放各种分类分组的个性化参数实体类", description = "表(cdp_parameter)的对应的实体类")
public class CdpParameterEntity extends AbstractSoftDeleteJpaEntity<Long, Long> implements Serializable {
    ...
    @Basic
    @ApiModelProperty(value = "参数分类：具体取值于字典表cdp_dictionary.id=10", required = true)
    ...
    @Basic
    @ApiModelProperty(value = "该值由后台维护，更改数据时前端不需要关心，取值于cdp_user.id")
