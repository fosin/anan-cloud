...
/**
 * 包含菜单、按钮两种权限(CdpSysPermission)更新DTO
 *
 * @author fosin
 ...
 */
@Data
@ApiModel(value = "包含菜单、按钮两种权限更新DTO", description = "表(cdp_sys_permission)的对应的更新DTO")
public class CdpSysPermissionUpdateDto implements Serializable {
    private static final long serialVersionUID = 110237401714730717L;
    ...
    @NotNull
    @ApiModelProperty(value = "权限类型：0=按钮、1=组件菜单，对应ur是前端组件l、2=链接菜单，对应url是http(s)链接地址、3=目录菜单，对应是目录菜单，具体取值于字典表cdp_sys_dictionary.id=13，当权限类型是1：组件菜单 3：目录菜单时表示该节点不是一个叶子节点", example = "Integer", required = true)
    ...
    @ApiModelProperty(value = "http请求方法：GET、POST、DELETE、OPTIONS、PUT、PATCH，具体取值于字典表cdp_sys_dictionary.id=12", example = "String")
