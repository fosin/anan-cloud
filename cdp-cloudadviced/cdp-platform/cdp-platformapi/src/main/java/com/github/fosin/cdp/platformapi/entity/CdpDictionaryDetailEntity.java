...
/**
 * 系统通用字典明细表(CdpDictionaryDetail)实体类
 *
 * @author fosin
...
@DynamicUpdate
@SQLDelete(sql = "update cdp_dictionary_detail set deleted = 1 where id = ?")
@Where(clause = "deleted = 0")
@Table(name = "cdp_dictionary_detail")
@ApiModel(value = "系统通用字典明细表实体类", description = "表(cdp_dictionary_detail)的对应的实体类")
public class CdpDictionaryDetailEntity extends AbstractSoftDeleteJpaEntity<Long, Long> implements Serializable {
    ...
    @Basic
    @ApiModelProperty(value = "取值于字典明细表cdp_dictionary.id", required = true)
