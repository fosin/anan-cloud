...
/**
 * 包含菜单、按钮两种权限(CdpPermission)创建DTO
 *
 * @author fosin
 ...
 */
@Data
@ApiModel(value = "包含菜单、按钮两种权限创建DTO", description = "表(cdp_permission)的对应的创建DTO")
public class CdpPermissionCreateDto implements Serializable {
    ...
    @NotNull
    @ApiModelProperty(value = "权限类型：0=按钮、1=组件菜单，对应ur是前端组件l、2=链接菜单，对应url是http(s)链接地址、3=目录菜单，对应是目录菜单，具体取值于字典表cdp_dictionary.id=13，当权限类型是1：组件菜单 3：目录菜单时表示该节点不是一个叶子节点", example = "Integer", required = true)
