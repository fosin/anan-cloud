...
@DynamicUpdate
@Table(name = "cdp_permission")
@ApiModel(value = "包含菜单、按钮两种权限实体类", description = "表(cdp_permission)的对应的实体类")
public class CdpPermissionEntity extends AbstractCreateUpdateJpaEntity<Long, Long> implements Serializable {
    ...
    private List<CdpPermissionEntity> children;
    ...
    @Basic
    @ApiModelProperty(value = "权限类型：0=按钮、1=组件菜单，对应ur是前端组件l、2=链接菜单，对应url是http(s)链接地址、3=目录菜单，对应是目录菜单，具体取值于字典表cdp_dictionary.id=13，当权限类型是1：组件菜单 3：目录菜单时表示该节点不是一个叶子节点", required = true)
    ...
    @Basic
    @ApiModelProperty(value = "http请求方法：GET、POST、DELETE、OPTIONS、PUT、PATCH，具体取值于字典表cdp_dictionary.id=12")
