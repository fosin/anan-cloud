...
/*完善用户信息*/
alter table anan_user add real_name_verified tinyint unsigned default 0 not null comment '实名认证标志' after username;
alter table anan_user add email_verified tinyint unsigned default 0 not null comment '邮箱认证标志' after email;
alter table anan_user add phone_verified tinyint unsigned default 0 not null comment '手机验证标志' after phone;
alter table anan_user add family_name varchar(32) default null comment '姓氏' after username;
alter table anan_user add middle_name varchar(64) default null comment '中间名' after family_name;
alter table anan_user add given_name varchar(32) default null comment '名字' after middle_name;
alter table anan_user add nickname varchar(32) default null comment '昵称' after given_name;
alter table anan_user add preferred_username varchar(32) default null comment '希望被称呼的名字' after nickname;
alter table anan_user add website varchar(256) default null comment '网站地址' after preferred_username;
...
(
    ...
    PRIMARY KEY (id)
) DEFAULT CHARSET = utf8mb4 collate = utf8mb4_general_ci COMMENT ='用户地址表';
...
(
    ...
    update_time datetime         NOT NULL COMMENT '修改日期',
    PRIMARY KEY (id)
) DEFAULT CHARSET = utf8mb4 collate = utf8mb4_general_ci COMMENT ='用户证件表';
...
(
    ...
    create_time   datetime         NOT NULL COMMENT '创建日期',
    PRIMARY KEY (id)
) DEFAULT CHARSET = utf8mb4 collate = utf8mb4_general_ci COMMENT ='用户第三方认证表';
...
(
    ...
    PRIMARY KEY (id)
) DEFAULT CHARSET = utf8mb4 collate = utf8mb4_general_ci COMMENT ='OAUTH2认证信息表';
...
(
    ...
    PRIMARY KEY (id)
) DEFAULT CHARSET = utf8mb4 collate = utf8mb4_general_ci COMMENT ='OAUTH2认证客户端注册表';
...
) DEFAULT CHARSET = utf8mb4 collate = utf8mb4_general_ci COMMENT ='OAUTH2认证信息对照表';

alter table oauth_approvals modify lastModifiedAt TIMESTAMP default current_timestamp() not null comment '修改时间';
alter table oauth_approvals add primary key (userId, clientId);
alter table oauth_code add primary key (code);
...
update anan_permission set path=concat(left(path, length(path) - 3), '*') where path like '%/api';

/*修复权限表数据，path机构相关的路径由organization修改成organiz */
update anan_permission set path=replace(path,'organization','organiz') where id in (4,5,6);

/*修复权限表数据，由于anan-data变更之后根据主键查询默认只支持GET方法，这里同步修复数据 */
update anan_permission set method='GET',update_time = current_timestamp where type = 5 and method in ('GET,POST', 'POST,GET') and path like '%/*';
