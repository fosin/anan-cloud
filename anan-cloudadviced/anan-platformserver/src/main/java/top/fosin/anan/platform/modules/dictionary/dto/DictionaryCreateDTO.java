...
@Schema(description = "系统通用字典表(anan_dictionary)创建DTO")
public class DictionaryCreateDTO {
    private static final long serialVersionUID = -96200070903677903L;
    
    @NotBlank(message = "字典名称" + "{jakarta.validation.constraints.NotBlank.message}", groups = Create.class)
    @Schema(description = "字典名称", example = "String")
    @Pattern(regexp = RegexUtil.SPECIAL, message = "名称不能包含特殊字符", groups = Create.class)
    private String name;

    @NotNull(message = "字典类别" + "{jakarta.validation.constraints.NotNull.message}", groups = Create.class)
    @Positive(message = "字典类别" + "{jakarta.validation.constraints.Positive.message}", groups = Create.class)
