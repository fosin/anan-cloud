...
public class OrganizationUpdateDTO extends Id<Long> {
    ...
    @NotNull(message = "父机构编码，取值于id，表示当前数据所属的父类机构" + "{javax.validation.constraints.NotNull.message}", groups = Update.class)
    @ApiModelProperty(value = "父机构编码，取值于id，表示当前数据所属的父类机构", required = true, example = "Long")
    private Long pid;

    @Positive(message = "顶级机构序号" + "{javax.validation.constraints.Positive.message}", groups = Update.class)
    @NotNull(message = "顶级机构序号" + "{javax.validation.constraints.NotNull.message}", groups = Update.class)
    @ApiModelProperty(value = "顶级机构编号：指用户注册的机构，通常是一个集团组的最高级别机构，新增和修改数据时必填!")
    private Long topId;

    @NotBlank(message = "机构编码" + "{javax.validation.constraints.NotBlank.message}", groups = Update.class)
    ...
    @NotBlank(message = "机构名称" + "{javax.validation.constraints.NotBlank.message}", groups = Update.class)
    ...
    @NotBlank(message = "机构全名" + "{javax.validation.constraints.NotBlank.message}", groups = Update.class)
