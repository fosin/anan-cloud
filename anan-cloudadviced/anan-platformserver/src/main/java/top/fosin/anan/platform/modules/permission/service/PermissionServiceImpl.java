package top.fosin.anan.platform.modules.permission.service;
...
import top.fosin.anan.platform.modules.permission.dao.PermissionDao;
...
public class PermissionServiceImpl extends PermissionServiceGrpc.PermissionServiceImplBase implements PermissionService {
    ...
    public PermissionDTO create(PermissionCreateDTO dto) {
        ...
        return BeanUtil.copyProperties(permissionDao.save(createEntity), PermissionDTO.class);
    }
    ...
    public void update(PermissionUpdateDTO dto, String[] ignoreProperties) {
        Long id = dto.getId();
    ...
    protected List<Permission> getUpdateCascadeChild(PermissionUpdateDTO originEntity, Permission updateEntity) {
        ...
        allByPid.forEach(entity -> {
            PermissionUpdateDTO entity2 = BeanUtil.copyProperties(entity, PermissionUpdateDTO.class);
    ...
    @Override
    public List<PermissionTreeDTO> findByPidAndVersionId(Long pid, Long versionId) {
        List<Permission> entities = permissionDao.findAllByPidAndVersionId(pid, versionId);
        return BeanUtil.copyProperties(entities, PermissionTreeDTO.class);
    }
...
//            builder.setServiceCode(serviceCode);
    ...
    public List<PermissionDTO> findByServiceCode(String serviceCode) {
        List<Permission> entities = permissionDao.findAllByServiceId(serviceDao.findOneByCode(serviceCode).getId());
        return BeanUtil.copyProperties(entities, PermissionDTO.class);
    }
    ...
    public List<PermissionDTO> findByServiceCodes(List<String> serviceCodes) {
        ...
        List<Permission> permissionEntities =
                permissionDao.findByServiceIdIn(services.stream().map(Service::getId).collect(Collectors.toList()));
        List<PermissionDTO> permissionDTOS = BeanUtil.copyProperties(permissionEntities, PermissionDTO.class);
        for (PermissionDTO permissionDTO : permissionDTOS) {
            for (Service serviceEntity : services) {
                if (serviceEntity.getId().equals(permissionDTO.getServiceId())) {
                    permissionDTO.setServiceCode(serviceEntity.getCode());
                }
            }
        }
