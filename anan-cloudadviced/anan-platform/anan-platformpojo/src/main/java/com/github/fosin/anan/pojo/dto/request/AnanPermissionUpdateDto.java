 ...
 */
@Data
@ApiModel(value = "包含菜单、按钮两种权限更新DTO", description = "表(anan_permission)的对应的更新DTO")
public class AnanPermissionUpdateDto implements Serializable {
    ...
    @ApiModelProperty(value = "权限类型：0=按钮、1=组件菜单，对应ur是前端组件l、2=链接菜单，对应url是http(s)链接地址、3=目录菜单，对应是目录菜单，具体取值于字典表anan_dictionary.code=13，当权限类型是1：组件菜单 3：目录菜单时表示该节点不是一个叶子节点", example = "Integer", required = true)
